        ...
        for idx, qid in enumerate(test_qids):
            ...
            if extract_pred != "FAILED":
                if extract_pred in args.options:
                    ...
                    extract_pred = secrets.SystemRandom().choice(range(0,len(args.options)))
            else:
                num_fail += 1
                extract_pred = secrets.SystemRandom().choice(range(len(args.options))) # random choose one option
